...
@storage_var
func answer_users_form(user_address : felt, id_form : felt, id_question : felt) -> (answer : felt):
end
...
}(user_address : felt) -> (records_len : felt, records : Row*):
    alloc_locals
    let (count : felt) = forms_count.read()
    let (records : Row*) = alloc()
    _recurse_my_score_forms_completed(user_address, 0, count, records, 0)
    let (count_forms_completed) = _recurse_count_my_score_forms_completed(
        user_address, 0, count, records
    )
